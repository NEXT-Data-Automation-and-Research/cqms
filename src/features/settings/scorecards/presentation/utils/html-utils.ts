...
export function createScorecardRowHTML(scorecard: any): string {
  ...
  const deleteButton = (scorecard.audit_count === 0 || typeof scorecard.audit_count === 'undefined')
    ? `<button onclick="window.scorecardController.deleteScorecard('${scorecard.id}', '${escape(scorecard.table_name)}')" class="btn-action btn-action-danger" title="Delete Scorecard">
    ...
    : `<button onclick="window.scorecardController.showCannotDeleteMessage('${escape(scorecard.name)}', ${scorecard.audit_count})" class="btn-action" style="background-color: #f3f4f6; color: #9ca3af; cursor: not-allowed;" title="Cannot delete: Contains ${scorecard.audit_count} audit report(s)">
        <svg style="width: 0.6562rem; height: 0.6562rem;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  return `
    <tr>
      <td>
        <div style="font-weight: 600; color: #1A733E;">${escape(scorecard.name)}</div>
      </td>
      <td>
      ...
      <td>
        <code style="font-size: 0.4688rem; background: #f3f4f6; padding: 0.0938rem 0.2812rem; border-radius: 0.1875rem; color: #374151;">${escape(scorecard.table_name)}</code>
      </td>
      <td>
      ...
      <td style="text-align: center;">
        <div style="display: flex; gap: 0.1875rem; justify-content: center;">
          <button onclick="window.scorecardController.viewScorecard('${scorecard.id}')" class="btn-action" title="View Details">
            <svg style="width: 0.6562rem; height: 0.6562rem;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
              <circle cx="12" cy="12" r="3"/>
            </svg>
          </button>
          <button onclick="window.scorecardController.editScorecard('${scorecard.id}')" class="btn-action btn-action-primary" title="Edit">
            <svg style="width: 0.6562rem; height: 0.6562rem;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
            </svg>
          </button>
          <button onclick="window.scorecardController.toggleStatus('${scorecard.id}', ${!scorecard.is_active})" class="btn-action ${scorecard.is_active ? 'btn-action-danger' : 'btn-action-success'}" title="${scorecard.is_active ? 'Deactivate' : 'Activate'}">
            ${scorecard.is_active 
