...
export function renderAuditDetailModalHTML(audit: AuditReport, parameters: ScorecardParameter[] = []): string {
  ...
  // Transcript container - reuses the ConversationPanel component from audit-form
  const transcriptHtml = `
    <div id="modalConversationContainer" data-interaction-id="${escapeHtml(String(interactionId))}" style="background: #f9fafb; border-radius: 0.375rem; padding: 0; border: 0.0352rem solid #e5e7eb; display: flex; flex-direction: column; height: 80vh; transition: height 0.3s ease;">
      ${interactionId ? `
        <div style="text-align: center; padding: 1.2937rem; color: #6b7280;">
          ...
          <p style="margin-top: 0.6469rem; font-size: 0.5659rem;">Loading conversation...</p>
        </div>
      ` : audit.transcript ? `
      ...
      <div style="background: ${headerGradient}; padding: 0.75rem 1.125rem; color: white; box-shadow: 0 0.1406rem 0.2109rem rgba(0,0,0,0.1);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(4.9219rem, 1fr)); gap: 0.375rem;">
          ...
          <div style="background: rgba(255,255,255,0.15); border-radius: 0.2812rem; padding: 0.375rem 0.5625rem; backdrop-filter: blur(0.3516rem);">
            ...
            <p style="font-size: 0.75rem; font-weight: 700; margin: 0; font-family: 'Poppins', sans-serif;">${escapeHtml(String(audit.averageScore ?? audit.average_score ?? (audit as any).averageScore ?? (audit as any).average_score ?? '0'))}%</p>
          </div>
          <div style="background: rgba(255,255,255,0.15); border-radius: 0.2812rem; padding: 0.375rem 0.5625rem; backdrop-filter: blur(0.3516rem);">
            ...
            <p style="font-size: 0.75rem; font-weight: 700; margin: 0; font-family: 'Poppins', sans-serif;">${escapeHtml(String(audit.totalErrorsCount ?? audit.total_errors_count ?? (audit as any).totalErrorsCount ?? (audit as any).total_errors_count ?? '0'))}</p>
          </div>
          <div style="background: rgba(255,255,255,0.15); border-radius: 0.2812rem; padding: 0.375rem 0.5625rem; backdrop-filter: blur(0.3516rem);">
