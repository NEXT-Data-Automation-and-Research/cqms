...
export class ConfirmationDialog {
  ...
  async show(options: ConfirmationDialogOptions): Promise<boolean> {
    ...
    // Create new promise
    this.currentPromise = new Promise<boolean>((resolve) => {
    ...
    this.dialog = dialogContainer.querySelector('.confirmation-dialog') as HTMLDivElement;
    ...
    // Show modal immediately (already visible with inline styles)

    // Focus management
    const confirmButton = dialogContainer.querySelector('[data-action="confirm"]') as HTMLButtonElement;
    if (confirmButton) {
    ...
    (this.overlay as any)._escapeHandler = escapeHandler;
   ...
   */
  private getIconBackgroundColor(type: string): string {
    switch (type) {
      case 'error':
      case 'confirm':
   ...
   */
  private getIconSVG(type: string, iconColor: string): string {
    switch (type) {
      case 'error':
      case 'confirm':
        return `
...
if (typeof window !== 'undefined') {
  (window as any).confirmationDialog = confirmationDialog;
}
